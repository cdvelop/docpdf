...
func (this *pdfWriter) ImportPage(reader *pdfReader, pageno int, boxName string) (int, error) {
	...
	// If requested canvas.Box name does not exist for this page, use an alternate canvas.Box
	...
	// If the requested canvas.Box name or an alternate canvas.Box name cannot be found, trigger an error
	// TODO: Improve error handling
	if _, ok := pageBoxes[boxName]; !ok {
		return -1, errs.New("canvas.Box not found: " + boxName)
	}
