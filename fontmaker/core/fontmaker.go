...
func (f *FontMaker) MakeDefinitionFile(gofontname string, mappath string, exportfile string, encode string, fontmaps []FontMap, info TtfInfo) (string, error) {
	...
	str += "import (\n"
	str += "	\"github.com/cdvelop/docpdf\"\n"
	str += ")\n"
	str += "type " + gofontname + " struct {\n"
	str += "\tfamily string\n"
	str += "\tfonttype string\n"
	str += "\tname string\n"
	str += "\tdesc  []docpdf.fontDescItem\n"
	str += "\tup int\n"
	str += "\tut int\n"
	str += "\tcw docpdf.fontCw\n"
	str += "\tenc string\n"
	str += "\tdiff string\n"
	str += "}\n"
	...
	str += "\treturn me.name\n"
	str += "}	\n"
	str += "func (me * " + gofontname + ")GetDesc() []docpdf.fontDescItem{\n"
	str += "\treturn me.desc\n"
	str += "}\n"
	...
	str += "\treturn me.ut\n"
	str += "}\n"
	str += "func (me * " + gofontname + ")GetCw() docpdf.fontCw{\n"
	str += "\treturn me.cw\n"
	str += "}\n"
...
func (f *FontMaker) MakeFontDescriptor(info TtfInfo) (string, error) {
	...
	fd += fmt.Sprintf("\tme.desc[0] =  docpdf.fontDescItem{ Key:\"Ascent\",Val : \"%d\" }\n", ascender)
	...
	fd += fmt.Sprintf("\tme.desc[1] =  docpdf.fontDescItem{ Key: \"Descent\", Val : \"%d\" }\n", descender)
	...
	fd += fmt.Sprintf("\tme.desc[3] =  docpdf.fontDescItem{ Key: \"Flags\", Val :  \"%d\" }\n", flags)
	...
	fd += fmt.Sprintf("\tme.desc[4] =  docpdf.fontDescItem{ Key:\"FontBBox\", Val :  \"[%d %d %d %d]\" }\n", fbb[0], fbb[1], fbb[2], fbb[3])

	// ItalicAngle
	fd += fmt.Sprintf("\tme.desc[5] =  docpdf.fontDescItem{ Key:\"ItalicAngle\", Val :  \"%d\" }\n", italicAngle)
	...
	fd += fmt.Sprintf("\tme.desc[6] =  docpdf.fontDescItem{ Key:\"StemV\", Val :  \"%d\" }\n ", stemv)
	...
	fd += fmt.Sprintf("\tme.desc[7] =  docpdf.fontDescItem{ Key:\"MissingWidth\", Val :  \"%d\" } \n ", missingWidth)
	return fd, nil
}
...
func (f *FontMaker) MakeWidthArray(widths map[int]int) (string, error) {
	...
	for c := 0; c <= 255; c++ {
		...
		if chr == "\"" {
			...
			str += "docpdf.toByte(\"" + chr + "\")"
		} else {
			str += fmt.Sprintf("docpdf.Chr(%d)", c)
		}
		str += fmt.Sprintf("]=%d\n", widths[c])
	}
	return str, nil
